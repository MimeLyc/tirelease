...
func SelectIssue(option *entity.IssueOption) (*[]entity.Issue, error) {
	...
	if err := storeGlobalDB.RawWrapper(sql, option).Find(&issues).Error; err != nil {
...
// 	if err := storeGlobalDB.DB.Delete(issue).Error; err != nil {
...
// 	// 存储
// 	if err := storeGlobalDB.DB.Clauses(clause.OnConflict{DoNothing: true}).Create(&issue).Error; err != nil {
...
func DeleteIssueByIssueID(issueId string) ([]entity.Issue, error) {
	...
	if err := storeGlobalDB.DB.Clauses(clause.Returning{}).Where(where).Delete(&issues).Error; err != nil {
		return nil, err
	}
	return issues, nil
}
