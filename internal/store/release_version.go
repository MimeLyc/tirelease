...
func SelectReleaseVersion(option *entity.ReleaseVersionOption) (*[]entity.ReleaseVersion, error) {
	...
	if err := storeGlobalDB.RawWrapper(sql, option).Find(&releaseVersions).Error; err != nil {
...
func CreateReleaseVersion(version *entity.ReleaseVersion) error {
	...
	// 存储
	if err := storeGlobalDB.DB.Omit("Repos", "Labels").Create(&version).Error; err != nil {
...
func UpdateReleaseVersion(version *entity.ReleaseVersion) error {
	...
	// 更新
	if err := storeGlobalDB.DB.Omit("CreateTime", "Repos", "Labels").Save(&version).Error; err != nil {
...
func DeleteReleaseVersionByName(name string) ([]entity.ReleaseVersion, error) {
	...
	if err := storeGlobalDB.DB.Clauses(clause.Returning{}).Where(where).Delete(&versions).Error; err != nil {
		return nil, err
	}
	return versions, nil
}
